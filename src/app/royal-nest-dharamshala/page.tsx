...
export default function RoyalNestDharamshalaPage() {
  ...
  const [popupData, setPopupData] = useState({ name: "", email: "", phone: "" });
  ...
  // ── existing brochure form handlers ──
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData((p) => ({ ...p, [name]: value }));
    if (errors[name]) setErrors((p) => ({ ...p, [name]: "" }));
  ...
  const handlePopupChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setPopupData((p) => ({ ...p, [name]: value }));
    if (popupErrors[name]) setPopupErrors((p) => ({ ...p, [name]: "" }));
  };

  const validatePopup = () => {
    const v: Record<string, string> = {};
    if (!popupData.name.trim()) v.name = "Name is required.";
    if (!popupData.email.trim()) v.email = "Email is required.";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(popupData.email)) v.email = "Enter a valid email.";
    if (!popupData.phone.trim()) v.phone = "Phone is required.";
    else if (!/^[\+]?[0-9]{7,15}$/.test(popupData.phone.replace(/\s/g, ""))) v.phone = "Enter a valid phone number.";
    setPopupErrors(v);
    return Object.keys(v).length === 0;
  };
...
const handlePopupSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    a.click();
    document.body.removeChild(a);
    setTimeout(() => setShowLeadPopup(false), 1800);
  } catch {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* ── LEAD POPUP — fires on every page load/refresh ── */}
      {showLeadPopup && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">

            {/* Green header */}
            <div className="bg-gradient-to-r from-[#14532d] to-[#15803d] px-5 py-4 flex items-center justify-between">
              <div>
                <div className="text-white font-bold text-sm tracking-wide">Royal Nest Forest View</div>
                <div className="text-green-200 text-xs mt-0.5">🏔 Dharamshala, Himachal Pradesh</div>
              </div>
              <button
                onClick={() => setShowLeadPopup(false)}
                className="text-white/70 hover:text-white text-xl leading-none p-1 rounded-full hover:bg-white/10 transition-colors"
            ...
            <div className="px-5 py-5 sm:px-6 sm:py-6">
              <h2 className="text-slate-900 text-xl font-bold mb-1">Download the Brochure</h2>
              <p className="text-slate-500 text-xs mb-5">
                Get floor plans, pricing &amp; master plan — instantly.
              </p>

              {popupSubmitted ? (
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-700 text-sm text-center">
                  ✓ Thank you! Your brochure is downloading now.
                </div>
              ) : (
                <form onSubmit={handlePopupSubmit} className="space-y-3">
                  <div>
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-slate-900 text-sm outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                    />
                    {popupErrors.name && <p className="mt-1 text-xs text-red-500">{popupErrors.name}</p>}
                  </div>
                  <div>
                    <input
                      name="email"
                      ...
                      className="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-slate-900 text-sm outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                    />
                    {popupErrors.email && <p className="mt-1 text-xs text-red-500">{popupErrors.email}</p>}
                  </div>
                  <div>
                    <input
                      name="phone"
                      ...
                      className="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-slate-900 text-sm outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                    />
                    {popupErrors.phone && <p className="mt-1 text-xs text-red-500">{popupErrors.phone}</p>}
                  </div>
                  <button
                    type="submit"
                    disabled={popupSubmitting}
                    className="w-full rounded-xl bg-slate-900 py-3 text-white text-sm font-semibold hover:bg-green-800 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {popupSubmitting ? "Sending…" : "Download Brochure →"}
                  </button>
                </form>
              )}

              <p className="text-center text-slate-400 text-[11px] mt-4">
                HP TCP Approved · RERA: HPRERAKAN2023036/P
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="relative w-full h-screen min-h-[500px] overflow-hidden">
        ...
        >
          ...
          {/* Two hero buttons: Download Brochure + Visit the Website */}
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs sm:max-w-none sm:w-auto justify-center">
            <button
              onClick={scrollToForm}
              className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-3.5 bg-white text-slate-900 font-semibold rounded-full hover:bg-slate-100 transition-all duration-200 text-sm tracking-wide shadow-lg"
            >
              Download Brochure
            </button>
            {/* <a
      ...
      <section className="py-12 sm:py-20 px-4 sm:px-6 bg-slate-900 text-white">
        ...
        <div className="max-w-3xl mx-auto mb-10 sm:mb-14">
          <div className="flex items-center gap-3 mb-5 sm:mb-6">
            ...
            <p className="text-[#7ec8e3] text-xs uppercase tracking-widest font-semibold px-2">Tower A</p>
            <div className="h-px flex-1 bg-white/10" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-8">
            {[
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center gap-3 mb-5 sm:mb-6">
            <div className="h-px flex-1 bg-white/10" />
            <p className="text-[#7ec8e3] text-xs uppercase tracking-widest font-semibold px-2">Tower B</p>
            <div className="h-px flex-1 bg-white/10" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-8">
            {[
